 ...
 */
export function searchCategories(query, lang = 'sv') {
    const lowerQuery = query.toLowerCase();
    return CATEGORIES.filter((cat) => {
        const translation = cat[lang];
        return (cat.id.toLowerCase().includes(lowerQuery) ||
 ...
 */
export function getCategoryTranslation(categoryId, lang = 'sv') {
    return CATEGORIES.find((cat) => cat.id === categoryId);
}
/**
 * Get DIS Phase 1 categories
 */
export function getDISFase1Categories() {
