 ...
 */
export function scoreRequiredFields(data, lang) {
    const items = [];
    const suggestions = [];
    // name: 5p
    items.push({
    ...
    const hasDescription = data.description && Object.keys(data.description).length > 0;
    items.push({
        name: 'description',
 ...
 */
export function scoreDocumentation(data, lang) {
    const items = [];
    const suggestions = [];
    const descriptions = Object.values(data.description || {});
    // longDescription >500 chars: 5p
    const hasLongDesc = descriptions.some((d) => d.longDescription && d.longDescription.length > 500);
    items.push({
    ...
    const hasFeatures = descriptions.some((d) => d.features && d.features.length >= 3);
    items.push({
        name: 'features',
 ...
 */
export function scoreLocalisation(data, lang) {
    const items = [];
    const suggestions = [];
    ...
    const hasSwedishContact = contacts.some((c) => (c.email && c.email.endsWith('.se')) ||
 ...
 */
export function scoreMaintenance(data, lang) {
    const items = [];
    const suggestions = [];
    // maintenance.type = 'internal' or 'contract': 5p
    const hasActiveType = data.maintenance?.type === 'internal' ||
    ...
    const hasContacts = (data.maintenance?.contacts && data.maintenance.contacts.length > 0) ?? false;
    items.push({
 ...
 */
export function scoreDISSpecific(data, lang) {
    const items = [];
    const suggestions = [];
    // Category matches DIS Phase 1: 5p
    const categories = data.categories || [];
    const hasDISCategory = categories.some((cat) => DIS_FASE1_CATEGORIES.includes(cat));
    items.push({
