...
export function validate(
  ...
  if (!valid && validateSchema.errors) {
    for (const err of validateSchema.errors as AjvError[]) {
      const path = err.instancePath.replace(/^\//, '').replace(/\//g, '.');
      const error = mapAjvError(err, lang);
      errors.push({
        path: path || (err.params?.missingProperty as string) || '',
        message: error,
      });
    }
  }
 ...
 */
export function parse(yaml: string): PublicCode {
  return parseYaml(yaml) as PublicCode;
}

/**
 * Map Ajv error to localized message
 */
function mapAjvError(err: AjvError, lang: Language): string {
  const field =
 ...
 */
function getWarnings(data: PublicCode, lang: Language): ValidationWarning[] {
  const warnings: ValidationWarning[] = [];
  ...
  if (!data.softwareVersion) {
    warnings.push({
      ...
      message:
        lang === 'sv' ? 'Lägg till softwareVersion' : 'Add softwareVersion',
    });
  }
