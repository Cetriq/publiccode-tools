...
import { validate, scoreYaml, type Language } from '@samhallskodex/core';
 ...
 */
async function fetchPubliccodeFromGitHub(
  owner: string,
  repo: string,
  branch = 'main'
): Promise<string | null> {
...
export const checkCommand = new Command('check')
  ...
  .action(async (repo, options) => {
    ...
    // Fetch the file
    const yaml = await fetchPubliccodeFromGitHub(
      parsed.owner,
      parsed.repo,
      options.branch
    );
    ...
    console.log(
      chalk.green(
        lang === 'sv' ? '✓ publiccode.yml hittades!\n' : '✓ publiccode.yml found!\n'
      )
    );
    ...
    if (!validation.valid) {
      console.log(
        chalk.red(lang === 'sv' ? '✗ Validering misslyckades' : '✗ Validation failed')
      );
      ...
      console.log(
        chalk.green(lang === 'sv' ? '✓ Validering lyckades!' : '✓ Validation passed!')
      );
    ...
    if (options.detailed) {
      ...
      for (const cat of categories) {
        ...
        const catColor =
          breakdown.score === breakdown.maxScore ? chalk.green : chalk.yellow;
        console.log(
          `  ${lang === 'sv' ? cat.sv : cat.en}: ${catColor(`${breakdown.score}/${breakdown.maxScore}`)}`
        );
        ...
        const priorityIcon =
          suggestion.priority === 'high'
            ...
            : suggestion.priority === 'medium'
              ...
              : chalk.dim('○');
        console.log(
          `  ${priorityIcon} ${suggestion.message} ${chalk.dim(`(+${suggestion.potentialPoints} ${lang === 'sv' ? 'poäng' : 'points'})`)}`
        );
      }
      if (result.suggestions.length > 5) {
        console.log(
          chalk.dim(
            `  ... ${lang === 'sv' ? 'och' : 'and'} ${result.suggestions.length - 5} ${lang === 'sv' ? 'till' : 'more'}`
          )
        );
      }
    }
